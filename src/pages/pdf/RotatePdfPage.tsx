...
export const RotatePdfPage: React.FC = () => {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <ToolHeader
        toolId="pdf-rotate"
        title="PDF 페이지 회전 (PDF Rotate)"
        description="PDF 문서의 각 페이지 썸네일을 시각적으로 확인하며 90도, 180도 원하는 각도로 개별/일괄 회전하여 저장합니다."
      />
      ...
      {resultUrl ? (
        <div className="glass-panel" style={{ padding: '2.5rem', borderRadius: 'var(--radius-lg)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem' }}>
          ...
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700 }}>PDF 회전 저장 완료!</h2>
        ...
        <div className="glass-panel" style={{ padding: '1.75rem', borderRadius: 'var(--radius-lg)', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
            <div>
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>총 {pageThumbs.length} 페이지</p>
            </div>

            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => handleRotateAll(90)} className="btn-secondary" style={{ fontSize: '0.85rem' }}>
                <RotateCw size={15} /> 전체 시계방향 (+90°)
              </button>
              <button onClick={() => handleRotateAll(-90)} className="btn-secondary" style={{ fontSize: '0.85rem' }}>
                <RotateCcw size={15} /> 전체 반시계방향 (-90°)
              </button>
          ...
          >
            <RotateCw size={18} /> {isProcessing ? t.processing : '회전 적용 후 PDF 저장'}
          </button>
        </div>
      )}
